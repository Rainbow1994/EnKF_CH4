...
class gosat_obs:
    ...
    def cor_factor(self, sea, lat):
        '''
        stratospheric bias factors according season and latitude
        '''
        if sea == 'DJF':
            cor_fc = ((-1.427569 * lat**2 + 9.904859 * lat) *1e-5 +0.987353)*1.04543897
        if sea == 'MAM':
            cor_fc = ((-1.069971 * lat**2 - 6.944126 * lat) *1e-5 +0.98045)*1.044742859
        if sea == 'JJA':
            cor_fc = ((-1.279419 * lat**2 + 12.800084 * lat) *1e-5 +0.986574)*1.04197637
        if sea == 'SON':
            cor_fc = ((-1.372728 * lat**2 + 41.036524 * lat) *1e-5 +0.990148)*1.04089757

        return cor_fc  

    #@nb.jit(nopython=True,parallel=True) 
    def prof_cor(self,tropp, cor_fc, org_prof, org_pres):
        '''
        stratospheric profiles correction
        Args:
            tropp: tropopause array
            cor_fc: correction factor array
            org_prof: gc tracer orginial 2-d profiles 
        '''
        n_site,n_pres = np.shape(org_prof)
        cor_prof = np.full_like(org_prof, np.nan)
        for i in range(n_site):
            cor_prof[i,:] = np.array([np.where(org_pres[i,j]<tropp[i], org_prof[i,j]*cor_fc[i], org_prof[i,j])\
    ...
    #@nb.jit(nopython=True,parallel=True) 
    def cor_array(self, lats):
        '''
        get correction factor arrays (gosat obs) at current date
        '''
        sea = self.get_season()

        cor_fc = []
        for lat in lats:
            cor_fc.append(self.cor_factor(sea,lat))
        cor_fc = np.array(cor_fc)

        return cor_fc

    def bias_cor(self, lons, lats, gc_pres, gc_profiles, tropp_sites):
        '''
        GC stratospheric bias correction using seasonal parameters

        Args:
            lons: longitudes array(gosat obs)
            lats: latitudes array(gosat obs)
            gc_pres: GC pressure levels
            gc_profiles: GC tracer profiles
            tropp_field: tropopause field 
        Return:
            cor_prf: GC tracer profiles after stratospheric bias correction
        '''

        ## step1: tropopause field sampling (lons,lats)-->tropp[:] 
#         tropp_sites =  self.setup_nc_daily_feild(combine_en = tropp_field,\
#                                                  lons = lons,\
#                                                  lats = lats)     

        ## step2: bias correction factors calculation(date, lat)-->cor_fc[:]
        cor_fc = self.cor_array(lats)


        ## step3: stratospheric profiles correction(tropp, cor_fc)--> cor_prf[:,:]

        tropp =tropp_sites['pres'].values/100.

        cor_prf = self.prof_cor(tropp, cor_fc, gc_profiles, gc_pres)
...
#         cur_step = 'ST'+ str(step).zfill(3)
    ...
    def gen_hm(self, tropp_field, sampl_file):
        '''
...
#         filename = self.sp_info['filepath'] + self.sp_info['filename'].replace('YYYYMMDD', date)
        filename = glob.glob(self.sp_info['filepath']+'/*'+date+'.bpch')[-1]
        print(filename)
        ...
        tropp_sites =  self.setup_nc_daily_feild(combine_en = tropp_field,\
                                                 lons = lons,\
                                                 lats = lats)
        sgl_data = self.bias_cor(lons, lats, model_pres, sgl_data, tropp_sites)
        ...
        for em_step in em_step_list:
            ...
            for var in var_list:
                ...
                em_data = self.bias_cor(lons, lats, model_pres, em_data, tropp_sites)
    ...
    def gen_hm0(self, tropp_field, hm_file, sampl_file):
        '''
        ...
        lats = sat_data['latitude'].values
            ...
            # resampling bpch output
#         filename = self.sp_info['filepath'] + self.sp_info['filename'].replace('YYYYMMDD', date)
        filename = glob.glob(self.sp_info['filepath']+'/*'+date+'.bpch')
        sp_ch4 = open_bpchdataset(filename,\
                                  diaginfo_file=self.sp_info['diaginfo_file'],\
                                  tracerinfo_file=self.sp_info['tracerinfo_file'])
        ...
        ## step6: caculation xgp result of single tracers[hm0]
        sgl_data = sp_mod[self.sp_info['var_prefix']].values 
        sgl_data = self.bias_cor(lons, lats, model_pres, sgl_data, tropp_field)
        sp_xgp = self.get_xgp(sat_pres, sat_ave, model_pres, sgl_data, pres, delta_p, method='log')
        hm0 = xgp0 + sp_xgp
        hm0 = np.array(hm0)
...
#     YYYY = 2010
#     date_st = datetime(YYYY,1,1) 
#     date_end = datetime(YYYY,12,31)
#     period = pd.date_range(start = date_st, end = date_end, freq= 'D')
#     gosat_info = {
#     'filepath': '/data/GOSAT/GOSAT-XCH4/v9.0/gosat_v9/YYYY/',
#     'filename': 'UoL-GHG-L2-CH4-GOSAT-OCPR-YYYYMMDD-fv9.0.nc',
#     'sampl_path': './rerun_GC/YYYY/', 
# #    'sampl_path': '/data/shzhu/global_inv/sampl_rlt/GOSAT/',
#     'sampl_file': 'gosat_cor.YYYYMMDD.nc'
#     }
#     sp_diagn_info = {
...
#     'tracerinfo_file' : '/data/shzhu/org_ch4/global_4x5/tracerinfo_sp.dat',
#     'var_prefix': 'IJ_AVG_S_CH4'
#     }
#     tropp_field = xr.open_dataset('/data/geos-chem/tropp/tropp_pres.nc')

#     for day in period:
#         print(day.strftime('%Y-%m-%d'))
#         gosat_sampl = gosat_obs(date = day,\
#                                 info = gosat_info,\
#                                 sp_diagn_info = sp_diagn_info ,\
#                                 en_info_list = False,\
#                                 en_item_table = False)
        
      
#         daily_tropp = tropp_field.sel(time = day)
#         sampl_path = gosat_info['sampl_path'].replace('YYYY', day.strftime('%Y'))
#         if not os.path.exists(sampl_path):
#             mkdir_cmd = 'mkdir ' + sampl_path
#             os.system(mkdir_cmd)
            
#         sampl_file = sampl_path + gosat_info['sampl_file'].replace('YYYYMMDD', day.strftime('%Y%m%d'))
#         gosat_sampl.gen_hm0(daily_tropp, sampl_file)
    date_st = config.date_st
    date_end = config.date_end
    period = pd.date_range(start = date_st, end = date_end, freq= 'D')
    tropp_field = xr.open_dataset('/data/geos-chem/tropp/tropp_pres.nc')#pres.tropp.2020.nc
    for day in period:
        print(day.strftime('%Y-%m-%d'))
        gosat_sampl = gosat_obs(date = day,\
        ...
        gosat_sampl.gen_hm(daily_tropp, sampl_file)
