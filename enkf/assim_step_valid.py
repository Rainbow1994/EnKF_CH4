...
import tccon as tc
import tccon_sites as tc_sites
...
class assim_step:
    ...
    def do_one_step(self, update_step = 0, rerun_step = 6, sample_step = 0):
        ...
        valid_tc = tc.tccon_obs(sel_id = tc_sites.valid_id)
        ...
        if (self.cur_step >= update_step):
            ...
            for idoy in range(cur_st_doy, cur_end_doy):
                ...
                if not self.sampl_input:
                    if (gcdf.sep_en):
                        sample_rlt = scg.get_hm2(cur_step = sample_step,\
                                                 doy = idoy,\
                                                 yyyy = yyyy,\
                                                 sampl_file = sampl_file)
                        
                    else:
                        sample_rlt = scg.get_hm(cur_step = sample_step,\
                                                doy = idoy,\
                                                yyyy = yyyy,\
                                                sampl_file = sampl_file)
                 
                else:
                    if os.path.exists(sampl_file):
                        sample_rlt = xr.open_dataset(sampl_file)
                    else:
                        sample_rlt = False
                ...
                ## ***** TCCON Validation 
                ## -----------------------------------------------------------
                ############ Daily tccon validation ############################
                
                ## read xgp0, xgp parameters related to TCCON observations    
                sampl_file = gcdf.tccon_file +  "." + time[:10] + ".nc"
                if not self.tccon_input:
                    if (gcdf.sep_en):
                        tccon_sample = valid_tc.tccon_hm2(cur_step = sample_step,\
                                                 doy = idoy,\
                                                 yyyy = yyyy,\
                                                 sampl_file = sampl_file)
                        
                    else:
                        tccon_sample = valid_tc.get_hm(cur_step = sample_step,\
                                                doy = idoy,\
                                                yyyy = yyyy,\
                                                sampl_file = sampl_file)
                    ...
                    if os.path.exists(sampl_file):
                        tccon_sample = xr.open_dataset(sampl_file)
                    else:
                        tccon_sample = False
                        
                ##  tccon observations  exit      
                if not tccon_sample:
                    
                    prior_mod = tccon_sample['hm0'].values          # model results(prior information) 
                    tccon_obs = tccon_sample['obs'].values            # observation results
                    tccon_h = tccon_sample['hm'].values                 # H: delta Yf_0/ delta Xf_0
                    tccon_err = tccon_sample['oerr'].values            # observation error
                    tccon_lon = tccon_sample['lon'].values              # longitudes of observation sites
                    tccon_lat = tccon_sample['lat'].values              # latitudes of observation sites
                    
                    ## calculate posterior conc at tccon sites using assimilated parameters 
                    tccon_mod_adj = dot(tccon_h, xinc)
                    post_conc = prior_mod + tccon_mod_adj
                    prior_err = dot(tccon_h, self.dx)#sqrt(diag(self.err))
                    prior_err = dot(prior_err, transpose(prior_err))
                    prior_err = sqrt(diag(prior_err))
                    inv_err = dot(tccon_h, tdx)
                    inv_err = dot(inv_err, transpose(inv_err))
                    inv_err = sqrt(diag(inv_err))
                    
                    ##  output validation result
                    outflnm = gcdf.tccon_valid_outpath + "/" + "tccon_valid" + "." + time[:10] + ".nc"
                    nobs = len(tccon_lon)
                    tccon_valid = xr.Dataset({'prior_conc': (['nobs'], prior_mod),
                                              'tccon_obs': (['nobs'], tccon_obs),
                                              'post_conc': (['nobs'], post_conc),
                                              'prior_err': (['nobs'], prior_err),
                                              'tccon_err': (['nobs'], tccon_err),
                                              'post_err': (['nobs'], inv_err),
                                              'tccon_lon': (['nobs'], tccon_lon),
                                              'tccon_lat': (['nobs'], tccon_lat)},
                                             coords = {'nobs': tccon['ns'].values})
                    tccon_valid.attrs['Description'] = 'TCCON Validation Result(ppbv)'
                    tccon_valid.attrs['tccon_lon'] = 'Longitudes of the TCCON observations'
                    tccon_valid.attrs['tccon_lat'] = 'Latitudes of the TCCON observations'
                    tccon_valid.attrs['prior_conc'] = 'Prior concentration at tccon sites(hm0)'
                    tccon_valid.attrs['tccon_obs'] = 'Observations(xch4)'
                    tccon_valid.attrs['post_conc'] = 'Poterior concentration at tccon sites'
                    tccon_valid.attrs['prior_err'] = 'Original conc errors'
                    tccon_valid.attrs['tccon_err'] = 'TCCON observation errors'
                    tccon_valid.attrs['post_er'] = 'Posterior conc errors'
                    tccon_valid.attrs['Date'] =  time
                    
                    tccon_valid.to_netcdf(outflnm)
            ...
            if (self.step_para_output):
                sstep = r'%2.2d' % (self.cur_step)
                outflnm = gcdf.inv_step_outpath + "/" + "step_assim_3" + "." + sstep + ".nc"
 ...
 #       self.mean_x_step.append(self.mean_x)
        ...
        if (self.cur_step >= rerun_step and self.cur_step != gcdf.step_end):
            ...
            if (gcdf.sep_en):
                rg.prepare_restart2(cur_step = self.cur_step,\
                                   doy_st = cur_st_doy,\
                                   doy_end = cur_end_doy,\
                                   yyyy = self.cur_yyyy,\
