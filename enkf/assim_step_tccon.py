...
class assim_step:
    ...
    def do_one_step(self, update_step = 0, rerun_step = 6, sample_step = 0):
        ...
        assim_tc = tc.tccon_obs(sel_id = tc_sites.assim_id)
        ...
        if (self.cur_step >= update_step):
            ...
            for idoy in range(cur_st_doy, cur_end_doy):
                ...
                if not self.sampl_input:
                    if (gcdf.sep_en):
                        sample_rlt = assim_tc.tccon_hm2(cur_step = sample_step,\
                                                      doy = idoy,\
                                                      yyyy = yyyy,\
                                                      sampl_file = sampl_file)
                    else:
                        sample_rlt = assim_tc.tccon_hm(cur_step = sample_step,\
                                                     doy = idoy,\
                                                     yyyy = yyyy,\
                                                     sampl_file = sampl_file)
                else:
                ...
                print(sample_rlt['hm0'],sample_rlt['hm'] )
                data_ctl_rlt = dqc.tccon_quality_ctl(sample_rlt)
                print(data_ctl_rlt)
            ...
            if (self.step_para_output):
                sstep = r'%2.2d' % (self.cur_step)
                outflnm = gcdf.inv_step_outpath + "/" + "step_assim_tccon_05" + "." + sstep + ".nc"
            ...
            incflnm = gcdf.inv_step_outpath + "/" + "step_assim_tccon_05" + "." + sstep + ".nc"
